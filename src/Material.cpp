...
void Material::loadTexture(const char* name) {  // Přidej parametr pro cestu, abys neměl pevně zakódovaný soubor
    ...
    glBindTexture(GL_TEXTURE_2D, textureID);  // <<-- Toto chybí! Bindni texturu před nastavením dat a parametrů
    ...
    glBindTexture(GL_TEXTURE_2D, 0);  // Unbind po nastavení (dobrý zvyk, aby ses vyhnul nechtěným změnám)
